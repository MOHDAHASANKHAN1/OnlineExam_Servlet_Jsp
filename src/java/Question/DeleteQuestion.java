...
public class DeleteQuestion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String queid = request.getParameter("delqueid");
        PrintWriter out = response.getWriter();
        try {
            if (queid != null) {
                Db_Connection db = new Db_Connection();
                String Query = "select * from quest_mstr where qid=?";
                String DelQuery = "delete from quest_mstr where qid=?";
                ...
                if (db.Rst.next()) {
                    ...
                    if (i > 0) {
                        response.sendRedirect("AdminHome?choice=ViewQuestionList&msg=Question Successfully Deleted&msgtype=delete");
                    } else {
                        response.sendRedirect("AdminHome?choice=ViewQuestionList&msg=Error In Question Deletion&msgtype=error");
                    }
                } else {
                    response.sendRedirect("AdminHome?choice=DeleteQuestion&msg=Wrong Question Id&msgtype=error");
                }
            } else {
                out.println("\n"
